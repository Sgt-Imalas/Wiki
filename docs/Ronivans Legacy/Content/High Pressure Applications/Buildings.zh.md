# 建筑
## 压缩泵排气口
内置压缩泵的加强型排气口，即使在超高压环境下也能排出气体。

向高压环境中排放气体。

必须与（高压气体管道）相连。

需要电力才能运行。
### 信息
| ![HPAVentGas](/assets/images/buildings/HPAVentGas.png){width = "200"} | |
|-|-|
|**尺寸:** | 宽 1 x 高 1|
|**耗电:**| 120 W|
|**科技:**| 暖通空调|

|**<font size="+1">建造材料:</font>**| |
|-|-|
|钢 或 硬化合金|200 kg|
|塑料|100 kg|

### 建筑接口
|输入:|输出:|
|-|-|
|气体 输入管道|-|


## 压缩泵排液口
内置压缩泵的加强型排液口，即使在超高压环境下也能排出液体。

向高压环境中排放液体。

必须与高压液体管道相连。

需要电力才能运行。
### 信息
| ![HPAVentLiquid](/assets/images/buildings/HPAVentLiquid.png){width = "200"} | |
|-|-|
|**尺寸:** | 宽 1 x 高 1|
|**耗电:**| 120 W|
|**科技:**| 改良水管|

|**<font size="+1">建造材料:</font>**| |
|-|-|
|钢 或 硬化合金|200 kg|
|塑料|100 kg|

### 建筑接口
|输入:|输出:|
|-|-|
|液体 输入管道|-|


## 减压气阀
一种机械阀门，能够将高压管道中的气体流量减小到正常管道的流量水平，从而避免管道破裂。

允许将气体从高压气体管道转输至普通气体管道。
### 信息
| ![DecompressionGasValve](/assets/images/buildings/DecompressionGasValve.png){width = "200"} | |
|-|-|
|**尺寸:** | 宽 2 x 高 1|
|**科技:**| 暖通空调|

|**<font size="+1">建造材料:</font>**| |
|-|-|
|钢 或 硬化合金|50 kg|
|塑料|20 kg|

### 建筑接口
|输入:|输出:|
|-|-|
|气体 输入管道|气体 输出管道|


## 减压液阀
一种机械阀门，能够将高压管道中的液体流量减小到正常管道的流量水平，从而避免管道破裂。

允许将液体从高压液体管道转输至普通液体管道。
### 信息
| ![DecompressionLiquidValve](/assets/images/buildings/DecompressionLiquidValve.png){width = "200"} | |
|-|-|
|**尺寸:** | 宽 2 x 高 1|
|**科技:**| 液体微调|

|**<font size="+1">建造材料:</font>**| |
|-|-|
|钢 或 硬化合金|50 kg|
|塑料|20 kg|

### 建筑接口
|输入:|输出:|
|-|-|
|液体 输入管道|液体 输出管道|


## 重型自动清扫器
自动清扫器的范围可以通过点击建筑查看。

通过吸走周围所有的杂物，实现清扫和供应任务的自动化。

材料会自动被运送至任何重型轨道装载器,重型轨道存放器、存储箱或所在范围内的一切建筑物中。

具有更大的范围，容量和转向速度。
### 信息
| ![HPA_TransferArm](/assets/images/buildings/HPA_TransferArm.png){width = "200"} | |
|-|-|
|**尺寸:** | 宽 3 x 高 1|
|**耗电:**| 960 W|
|**科技:**| 固体控制|

|**<font size="+1">建造材料:</font>**| |
|-|-|
|钢 或 硬化合金|400 kg|
|塑料|100 kg|


## 重型轨道桥
将轨道系统进行分区有助于确保物资能够准确送达指定目的地。

允许重型轨道和隔热重型轨道分层铺设且不自动连接。

不可穿过墙体和地面。
### 信息
| ![HPA_SolidRailBridge](/assets/images/buildings/HPA_SolidRailBridge.png){width = "200"} | |
|-|-|
|**尺寸:** | 宽 3 x 高 1|
|**科技:**| 大容量固体运输|

|**<font size="+1">建造材料:</font>**| |
|-|-|
|钢 或 硬化合金|100 kg|
|透明体|50 kg|

### 建筑接口
|输入:|输出:|
|-|-|
|-|固体 输出轨道|


## 重型轨道接合板
这种接合板能够将重型轨道穿过墙壁，同时不会导致气体或液体泄漏。

允许重型轨道和隔热重型轨道穿越墙壁和地板。

像隔热砖一样隔热。
### 信息
| ![HPA_SolidRailBridgeTile](/assets/images/buildings/HPA_SolidRailBridgeTile.png){width = "200"} | |
|-|-|
|**尺寸:** | 宽 1 x 高 1|
|**科技:**| 大容量固体运输|

|**<font size="+1">建造材料:</font>**| |
|-|-|
|矿物原料|400 kg|
|钢 或 硬化合金|100 kg|
|透明体|50 kg|

### 建筑接口
|输入:|输出:|
|-|-|
|-|固体 输出轨道|


## 重型轨道装载器
材料筛选项可用于决定哪些材料会被送到轨道上。

将大量固体材料装载到物流运输轨道上进行运输。

只装载所选资源。
### 信息
| ![HPA_InBox](/assets/images/buildings/HPA_InBox.png){width = "200"} | |
|-|-|
|**尺寸:** | 宽 1 x 高 3|
|**耗电:**| 960 W|
|**科技:**| 大容量固体运输|
|**存储容量:**| 10 t|

|**<font size="+1">建造材料:</font>**| |
|-|-|
|钢 或 硬化合金|300 kg|
|塑料|100 kg|

### 建筑接口
|输入:|输出:|
|-|-|
|-|固体 输出轨道|


## 重型运输轨道
轨道通过将物资运送到最需要的地方，从而省去了复制人们的奔波之苦。

在重型轨道装载器和轨道滑槽之间的轨道上运输大量的固体材料。

不可穿过墙体和地面。
### 信息
| ![HPA_SolidRail](/assets/images/buildings/HPA_SolidRail.png){width = "200"} | |
|-|-|
|**尺寸:** | 宽 1 x 高 1|
|**科技:**| 大容量固体运输|

|**<font size="+1">建造材料:</font>**| |
|-|-|
|钢 或 硬化合金|100 kg|


## 重型轨道压合器
通过压缩来提高轨道运输量!

将来自轨道的固体材料合并并输出至重型运输轨道。
### 信息
| ![HPA_RailMerger](/assets/images/buildings/HPA_RailMerger.png){width = "200"} | |
|-|-|
|**尺寸:** | 宽 3 x 高 2|
|**耗电:**| 60 W|
|**科技:**| 大容量固体运输|

|**<font size="+1">建造材料:</font>**| |
|-|-|
|钢 或 硬化合金|250 kg|
|塑料|150 kg|

### 建筑接口
|输入:|输出:|
|-|-|
|固体 输入轨道|固体 输出轨道|
|固体 输入轨道|-|
|固体 输入轨道|-|


## 重型轨道筛选器
除所选的固体外，其他所有固体都会被送入输出运输轨道。

将一个固体材料从传送带上分离出来，并将其送入专用的运输轨道中。

兼容各种类型的运输轨道。
### 信息
| ![HPA_MultiSolidFilter](/assets/images/buildings/HPA_MultiSolidFilter.png){width = "200"} | |
|-|-|
|**尺寸:** | 宽 3 x 高 1|
|**耗电:**| 360 W|
|**科技:**| 固体管理|

|**<font size="+1">建造材料:</font>**| |
|-|-|
|钢 或 硬化合金|200 kg|
|塑料|100 kg|

### 建筑接口
|输入:|输出:|
|-|-|
|-|固体 输出轨道|


## 重型轨道减压阀
一种机械阀门，能够在重型轨道向运输轨道运输的时候减少资源流量，从而避免物品掉落。

允许将固体材料从重型运输轨道转移到普通的运输轨道。
### 信息
| ![HPA_SolidRailValve](/assets/images/buildings/HPA_SolidRailValve.png){width = "200"} | |
|-|-|
|**尺寸:** | 宽 1 x 高 2|
|**科技:**| 大容量固体运输|

|**<font size="+1">建造材料:</font>**| |
|-|-|
|钢 或 硬化合金|50 kg|
|塑料|20 kg|

### 建筑接口
|输入:|输出:|
|-|-|
|固体 输入轨道|固体 输出轨道|


## 高压气体管道
一根经过加固的气体管道，能够承受高压流量。
该管道的复合材质可确保气体在传输过程中温度变化不大。

最多可承载10kg的气体，并且温度变化极小。

可以穿过墙壁和地板。
### 信息
| ![HighPressureGasConduit](/assets/images/buildings/HighPressureGasConduit.png){width = "200"} | |
|-|-|
|**尺寸:** | 宽 1 x 高 1|
|**科技:**| 改良通风|

|**<font size="+1">建造材料:</font>**| |
|-|-|
|钢 或 硬化合金|10 kg|
|塑料|5 kg|


## 高压气体管道桥
一种经过加固的气体管道桥，能够承受高压流量。管道的复合材质可确保气体在运输过程中温度变化不大。

将一段高压气体管道覆盖在另一段之上，但不将其连接起来。

可穿过墙体和地面。
### 信息
| ![HighPressureGasConduitBridge](/assets/images/buildings/HighPressureGasConduitBridge.png){width = "200"} | |
|-|-|
|**尺寸:** | 宽 3 x 高 1|
|**科技:**| 改良通风|

|**<font size="+1">建造材料:</font>**| |
|-|-|
|钢 或 硬化合金|10 kg|
|塑料|5 kg|

### 建筑接口
|输入:|输出:|
|-|-|
|-|气体 输出管道|


## 高压气体管道筛选器
除所选的气体外，其他所有气体都会被送入输出管道中。

从混合气体中过滤所有选定的气体并送入到专用的筛选输出管道口。

兼容所有类型的气体管道。
### 信息
| ![HPA_MultiGasFilter](/assets/images/buildings/HPA_MultiGasFilter.png){width = "200"} | |
|-|-|
|**尺寸:** | 宽 3 x 高 1|
|**耗电:**| 360 W|
|**科技:**| 泵小型化|

|**<font size="+1">建造材料:</font>**| |
|-|-|
|钢 或 硬化合金|200 kg|
|塑料|100 kg|

### 建筑接口
|输入:|输出:|
|-|-|
|气体 输入管道|气体 输出管道|


## 高压气泵
一种先进的泵，够通过机械作用来压缩和输送气体。比标准泵更强大，能够输送大量气体，但这一功能仅通过高压气体管道实现。

抽取气体并将其通过高压气体管道输送出去。

必须完全浸没在气体中。
### 信息
| ![PressureGasPump](/assets/images/buildings/PressureGasPump.png){width = "200"} | |
|-|-|
|**尺寸:** | 宽 2 x 高 3|
|**耗电:**| 2400 W|
|**科技:**| 泵小型化|

|**<font size="+1">建造材料:</font>**| |
|-|-|
|钢 或 硬化合金|400 kg|
|塑料|200 kg|

### 建筑接口
|输入:|输出:|
|-|-|
|-|气体 输出管道|


## 高压液体管道桥
一种经过加固的液体管道桥梁，能够承受高压水流。管道的复合材质可确保液体在运输过程中温度变化不大。

将一段高压液体管道覆盖在另一段之上，但不将其连接起来。

可穿过墙体和地面。
### 信息
| ![HighPressureLiquidConduitBridge](/assets/images/buildings/HighPressureLiquidConduitBridge.png){width = "200"} | |
|-|-|
|**尺寸:** | 宽 3 x 高 1|
|**科技:**| 改良水管|

|**<font size="+1">建造材料:</font>**| |
|-|-|
|钢 或 硬化合金|10 kg|
|塑料|5 kg|

### 建筑接口
|输入:|输出:|
|-|-|
|-|液体 输出管道|


## 高压液体管道筛选器
除所选的液体外，其他所有液体都会被送入输出管道中。

从混合液体中过滤所有选定的液体并送入到专用的筛选输出管道口。

兼容所有类型的液体管道。
### 信息
| ![HPA_MultiLiquidFilter](/assets/images/buildings/HPA_MultiLiquidFilter.png){width = "200"} | |
|-|-|
|**尺寸:** | 宽 3 x 高 1|
|**耗电:**| 360 W|
|**科技:**| 泵小型化|

|**<font size="+1">建造材料:</font>**| |
|-|-|
|钢 或 硬化合金|200 kg|
|塑料|100 kg|

### 建筑接口
|输入:|输出:|
|-|-|
|液体 输入管道|液体 输出管道|


## 高压液体管道
一种经过强化的液体管道，能够承受高压流体。该管道的复合材质可防止液体在传输过程中温度发生显著变化。

最多可承载40kg的液体，并且温度变化极小。

可以穿过墙和地板。
### 信息
| ![HighPressureLiquidConduit](/assets/images/buildings/HighPressureLiquidConduit.png){width = "200"} | |
|-|-|
|**尺寸:** | 宽 1 x 高 1|
|**科技:**| 改良水管|

|**<font size="+1">建造材料:</font>**| |
|-|-|
|钢 或 硬化合金|10 kg|
|塑料|5 kg|


## 高压液泵
一种先进的泵，够通过机械作用来压缩和输送液体。比标准泵更强大，能够输送大量液体，但这一功能仅通过高压气体管道实现。

抽取液体并将其通过高压液体管道输送出去。

必须完全浸没在液体中。
### 信息
| ![PressureLiquidPump](/assets/images/buildings/PressureLiquidPump.png){width = "200"} | |
|-|-|
|**尺寸:** | 宽 2 x 高 3|
|**耗电:**| 960 W|
|**科技:**| 泵小型化|

|**<font size="+1">建造材料:</font>**| |
|-|-|
|钢 或 硬化合金|400 kg|
|塑料|200 kg|

### 建筑接口
|输入:|输出:|
|-|-|
|-|液体 输出管道|


## 隔热重型轨道
轨道通过将物资运送到最需要的地方，从而省去了复制人们的奔波之苦。

运输大量固体材料。

不可穿过墙体和地面。

运输的物品均完全隔热且不会漏气。
### 信息
| ![HPA_SolidRail_Insulated](/assets/images/buildings/HPA_SolidRail_Insulated.png){width = "200"} | |
|-|-|
|**尺寸:** | 宽 1 x 高 1|
|**科技:**| 大容量固体运输|

|**<font size="+1">建造材料:</font>**| |
|-|-|
|钢 或 硬化合金|125 kg|
|透明体|25 kg|


